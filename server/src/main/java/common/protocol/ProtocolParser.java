...
/**
 * Parses the line-based protocol:
 * - TEXT|<username>|<message>
 * - IMAGE|<username>|<filename>|<size>
 * - OK|<context>|<detail>
 * - ERR|<context>|<code>|<message>
 */
public final class ProtocolParser {
    public ProtocolMessage parseLine(String line) {
        ...
        if (trimmed.startsWith("IMAGE|")) {
            return parseImage(trimmed);
        }
        if (trimmed.startsWith("OK|")) {
            return parseOk(trimmed);
        }
        if (trimmed.startsWith("ERR|")) {
            return parseErr(trimmed);
        }
        return null;
    }
